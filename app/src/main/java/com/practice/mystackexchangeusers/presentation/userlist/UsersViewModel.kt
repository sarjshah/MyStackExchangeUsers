...
class UsersViewModel @Inject constructor(
    ...
    private val _rawList = MutableStateFlow(emptyList<User>())
    ...
    fun textChanged(text: String) {
        if (text.isNotEmpty()) {
            ...
            _viewState.value = ViewState.Content(_rawList.value.prepareForView(USER_FETCH_SIZE))
            _viewState.value = ViewState.Filtered(false)
        }
    }

    private val _filteredResult = queryStateFlow.combine(_rawList) { query, users ->
    ...
    private fun loadContent() =
        useCases.getUsersUseCase().onEach { result ->
            _viewState.value = when (result) {
                ...
                is Resource.Success -> {
                    _rawList.value = result.data ?: emptyList()
                    ViewState.Content(
    ...
    private fun List<User>.prepareForView(userFetchSize: Int) =
        sortedBy { it.userName }.take(userFetchSize)
